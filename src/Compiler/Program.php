...
class Program extends Pattern
{
     ...
     * @inheritdoc
     */
    public function compile(Compiler $c, Gen $g, &$bindings) : Results
    {
    ...
    protected function compile_globals(Compiler $c, Gen $g, array $bindings)
    {
        $results = $c->results();
        $globals = [];
        ...
        foreach ($bindings as $binding) {
            ...
            $sub_result->add_class($g->closure_class($class_name, $sub_result->flush_methods()));
    ...
    protected function compile_init_globals(Compiler $c, Gen $g, array $bindings, array $globals)
    {
        return array_flatten(
            ...
            array_map(
                function (Lang\Binding $binding) use ($g) {
                    $closure_name = $binding->variable()->name();
                    return [
                        [$g->stmt("\$free_vars_$closure_name = array()")],
                        array_map(
                            function (Lang\Variable $free_var) use ($g, $closure_name) {
